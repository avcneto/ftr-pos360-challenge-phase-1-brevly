...
const originalUrlResponseSchema = z
  .object({
    id: z.string(),
...
export const linkRouter: FastifyPluginAsyncZod = async (server) => {
  server.get(
    LinkRoutes.GET_ORIGINAL_BY_SHORT_URL,
    {
      schema: {
        ...
        tags: ["Links"],
        querystring: originalUrlQuerySchema,
        response: {
      ...
      schema: {
        ...
        response: {
          200: z
            .string()
            .describe(
              "CSV content with columns: id, original_url, short_url, access_count, created_at",
            )
            .meta({
  ...
  server.post(
    LinkRoutes.CREATE,
    {
      schema: {
        ...
        description:
          "Creates a new short link with accessCount initialized to 0.",
        tags: ["Links"],
        body: linkBodySchema,
        response: {
    ...
    async (request, reply) => {
      ...
      if (existingLink) {
        return reply
          .status(409)
          .send({ message: SHORT_URL_ALREADY_EXISTS_MESSAGE });
      }
  ...
  server.patch(
    LinkRoutes.INCREMENT_ACCESS_BY_ID,
    {
      schema: {
        ...
        description:
          "Increments the accessCount field by +1 for the given link ID.",
        tags: ["Links"],
  ...
  server.delete(
    ...
    async (request, reply) => {
      ...
      return reply
        .status(200)
        .send({ message: LINK_DELETED_SUCCESSFULLY_MESSAGE });
    },
  );
};
